			     ...
			     Indirect *branch)
{
	int n = 0;
	int i;
	int parent = episode_new_block(inode);

	branch[0].key = cpu_to_block(parent);
	if (parent) for (n = 1; n < num; n++) {
		struct buffer_head *bh;
		/* Allocate the next block */
		int nr = episode_new_block(inode);
		if (!nr)
			break;
	...
	for (i = 1; i < n; i++)
		bforget(branch[i].bh);
	for (i = 0; i < n; i++)
		episode_free_block(inode, block_to_cpu(branch[i].key));
	return -ENOSPC;
}
...
changed:
	...
	for (i = 1; i < num; i++)
		bforget(where[i].bh);
	for (i = 0; i < num; i++)
		episode_free_block(inode, block_to_cpu(where[i].key));
	return -EAGAIN;
}
...
unsigned episode_blocks(loff_t size, struct super_block *sb)
{
